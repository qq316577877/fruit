...
public final class MoneyUtil {
    ...
    public static String toChinese(String str) {
        ...
        String chineseDecimal = getChineseDecimal(decimals);
        if (null == chineseDecimal || chineseDecimal.length() == 0) {
            return getChineseInteger(integers, isMust5) + "整";
        }
        return getChineseInteger(integers, isMust5) + chineseDecimal;
    }
    ...
    public static String toSimplified(String chnAmt) {
        ...
        for (int i = 0; i < amtArrChar.length; i++) {
            if (mapping.containsKey(String.valueOf(amtArrChar[i]))) {
                amtArrChar[i] = mapping.get(String.valueOf(amtArrChar[i])).charAt(0);
            }
        }
