...
<script type="text/javascript">
    ...
    option = {
        ...
        tooltip: {
            ...
            formatter: '{b} <br/> {a0} : {c0} (万) <br/> {a1} : {c1} (万)<br/> {a2} : {c2} %<br/> {a3} : {c3} % '
        },
        grid: {
            left: '10%'
        },
        ...
        legend: {
            data:['订单金额','贷款金额','订单金额周环比','贷款金额周环比']
        },
        xAxis: [
            {
                type: 'category',
            ...
            },
            {
                ...
                label: {
                    normal: {
                        ...
                        position: 'top',
                        formatter: '{c} %',
                        offset:[-8,5]//是否对文字进行偏移。默认不偏移。例如：[30, 40] 表示文字在横向上偏移 30，纵向上偏移 40。
                    }
                },
                yAxisIndex: 1,
...
<script>
    ...
    function getStatOrderLoanWeekModelList(){
        $.ajax("${BASEPATH}/statistics/listStatCycleRingOfWeeks", {
...
//            async:false,
            ...
            if (data.code==200) {
                ...
                if(dataSay){
                    ...
                    refreshChartByData(cycleDatas,orderAmountDatas,loanAmountDatas,orderWowDatas,loanWowDatas,
     ...
     *根据数据刷新图形
     */
    function refreshChartByData(cycle,orderAmountDatas,loanAmountDatas,orderWowDatas,loanWowDatas,
        ...
        option.series[2].data = orderWowDatas;//订单周环比

        option.series[3].data = loanWowDatas;//贷款周环比
...
<#--此页面不需要登录即可访问   http://www.fruit.com/admin/statistics/statOrderLoanCycleRingWeeks-->
